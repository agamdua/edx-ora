...
class StaffLocation(LocationCapsule):
    ...
    def minimum_to_score(self):
        ...
        min_scored_for_location = control.minimum_to_use_peer
        location_ml_count = self.location_submissions().filter(preferred_grader_type="ML").count()

        # If there are any AI graded submissions for the location, increase the minimum.
        if location_ml_count > 0:
            min_scored_for_location = control.minimum_to_use_ai
        return min_scored_for_location
